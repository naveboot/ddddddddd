          ...
          >
            <div className="flex items-center space-x-3">
              ...
              <div className="text-left">
                ...
                <p className={`text-xs capitalize ${getRoleColor(currentOrganization?.role || '')}`}>
                  {currentOrganization?.role}
                </p>
              </div>
            </div>
            <ChevronDown size={16} className={`text-slate-400 transition-transform ${showOrganizations ? 'rotate-180' : ''}`} />
          </button>

          {/* Organizations Dropdown */}
          {showOrganizations && (
            <div className="absolute top-full left-4 right-4 mt-2 bg-slate-800 rounded-xl border border-slate-700 shadow-2xl z-50 max-h-80 overflow-y-auto">
              <div className="p-2">
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <div className="bg-gradient-to-br from-blue-400 to-indigo-500 rounded-lg p-2">
                        <span className="text-white font-bold text-xs">{org.avatar}</span>
                      </div>
                      <div className="text-left">
                        <p className="text-white font-medium text-sm">{org.name}</p>
                        <div className="flex items-center space-x-2">
                          <span className={`text-xs capitalize ${getRoleColor(org.role)}`}>
                            {org.role}
                          </span>
                          <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${getPlanBadge(org.plan)}`}>
                            {org.plan}
                          </span>
                        </div>
                        <p className="text-xs text-slate-400">{org.memberCount} membres</p>
                      </div>
                    </div>
                ...
                <div className="border-t border-slate-700 mt-2 pt-2">
                  <button 
                    onClick={() => {
                      handleNavigation('join-organization');
                      setShowOrganizations(false);
                    }}
                    className="w-full flex items-center space-x-3 p-3 rounded-lg hover:bg-slate-700/50 transition-colors text-slate-300"
                  >
                    <UserPlus size={16} />
                    <span className="text-sm">{t('joinOrganization')}</span>
                  </button>
                </div>
              </div>
            </div>
